 ...
 *
 * Based on the Code from Neil Kolban:
 * https://github.com/nkolban/esp32-snippets/blob/master/hardware/infra_red/receiver/rmt_receiver.c
 * Based on the Code from pcbreflux:
 * https://github.com/pcbreflux/espressif/blob/master/esp32/arduino/sketchbook/ESP32IRRemote/ir_demo/ir_demo.ino
 * Based on the Code from Xplorer001:
 * https://github.com/ExploreEmbedded/ESP32_RMT
 */

/* This is a simple code to receive and then save IR received by an IR sensor connected to the
 * ESP32, reason that I could not find any complete examples of IR send and Received in the RAW
 * format, I am hoping this code can then be used to build on the existing IR libraries out there.
 */
...
void ESP32IR::recvPIN(int recvpin, int port)
{
...
void ESP32IR::recvPIN(int recvpin)
{
    recvPIN(recvpin, (int)RMT_CHANNEL_0);
}

void ESP32IR::sendPIN(int sendpin)
{
    recvPIN(sendpin, (int)RMT_CHANNEL_0);
}

void ESP32IR::sendPIN(int sendpin, int port)
{
...
void ESP32IR::sendIR(unsigned int data[], int IRlength)
{
    rmt_config_t config;
    config.channel = (rmt_channel_t)rmtport;
...
void ESP32IR::getDataIR(rmt_item32_t item, unsigned int *datato, int index)
{
    ...
    Serial.print(highValue, HEX);
    Serial.print(",");
    datato[index + 1] = highValue;
}

void ESP32IR::buildItem(rmt_item32_t &item, int high_us, int low_us)
{
    item.level0 = 1;
...
void ESP32IR::decodeRAW(rmt_item32_t *data, int numItems, unsigned int *datato)
{
    int x = 0;
...
int ESP32IR::readIR(unsigned int *data, int maxBuf)
{
