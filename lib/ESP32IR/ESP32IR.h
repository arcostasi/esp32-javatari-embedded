 ...
 * Based on the Code from pcbreflux: https://github.com/pcbreflux/espressif/blob/master/esp32/arduino/sketchbook/ESP32IRRemote/ir_demo/ir_demo.ino
...
class ESP32IR
{
    public:
        ESP32IR();
        void recvPIN(int recvpin);
        void sendPIN(int sendpin);
        void recvPIN(int recvpin, int port);
        void sendPIN(int sendpin, int port);
        void initReceive();
        void initSend();
        void stopIR();
